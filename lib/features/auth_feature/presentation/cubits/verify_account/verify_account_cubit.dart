...
class VerifyAccountCubit extends Cubit<IVerifyAccountState> {
  ...
  VerifyAccountCubit(
    this._verifyAccountUseCase, SaveTokenUseCase saveTokenUseCase, SaveUserUseCase saveUserUseCase,
  ...
  void verifyAccount(VerifyAccountParams params) async {
    ...
    user.fold((failure) => emit(VerifyAccountFailureState(failure: failure)), (
      auth,
    ) async {
